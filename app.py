...
with k1:
    prog_prom = df_filt['Progreso'].mean() if not df_filt.empty else 0
    st.metric("Progreso Global", f"{prog_prom:.1%}")
with k2:
...
if not df_filt.empty:
    # Configuración de Escala (Zoom)
    if escala == "Días":
        t_format, t_tick = "%d %b", "D1"
        mostrar_finde = True
    elif escala == "Semanas":
        t_format, t_tick = "Sem %W", 604800000.0  # 7 días
        mostrar_finde = True
    else: # Meses
